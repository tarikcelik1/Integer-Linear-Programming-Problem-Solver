...
def branch(problem, best_solution, best_value, depth=0, max_depth=100):
    ...
    # Check integrality
    fractional_indices = [i for i, x in enumerate(solution) if not np.isclose(x, round(x), atol=1e-9)]
    ...
    new_row = np.zeros(A.shape[1])
    new_row[i] = 1
    A_lower = np.vstack([A, new_row])
